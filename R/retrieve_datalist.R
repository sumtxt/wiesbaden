...
retrieve_datalist <- function(tableseries,
	genesis=NULL, language='de', restapi = FALSE, ... ) {
	...
	if(restapi){
	  ...
	  baseurl <- paste(set_db(db=genesis['db'], restapi), "catalogue/cubes", sep="")

	  param <- list(
	    username  = genesis['user'],
	    password = genesis['password'],
	    selection = tableseries,
	    area = "all",
	    pagelength = '500',
	    language = "de")
	  ...
	  if(is.null(jsondata$List)) return("No results found.")

	  d <- data.frame(matrix(unlist(jsondata$List), ncol = max(lengths(jsondata$List)), byrow = TRUE))
	  colnames(d) <- names(jsondata$List[[1]])
